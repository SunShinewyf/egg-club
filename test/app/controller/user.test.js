...
describe('test/app/controller/user.test.js', () => {
  ...
  it('should GET /register', async () => {
    const result = await request(app.callback()).get('/users/register').expect(200);
  ...
  it('should GET /login', async () => {
    const result = await request(app.callback()).get('/users/login').expect(200);
    const $ = cheerio.load(result.text);
  ...
  it('should GET /setting', async () => {
    const result = await request(app.callback()).get('/users/setting').expect(200);
    const $ = cheerio.load(result.text);
